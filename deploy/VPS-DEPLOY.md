...
- `build-goodness-images-image.ps1`：`package-for-vps.ps1` 的别名入口，命名风格更接近 `new-api/scripts`。
- `run-on-vps.sh`：在 VPS 上导入 tar 包并启动容器。
- `deploy-goodness-images.sh`：`run-on-vps.sh` 的别名入口。
...
```powershell
.\deploy\build-goodness-images-image.ps1
```
...
```bash
scp -r deploy/bundle user@your-vps:/opt/goodness-images
```

## VPS 启动

```bash
cd /opt/goodness-images/bundle
...
```bash
chmod +x deploy-goodness-images.sh
./deploy-goodness-images.sh
```

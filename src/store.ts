...
const defaultCustomSettings = mergeSettings(DEFAULT_SETTINGS, {})
...
export const useStore = create<AppState>()(
  persist(
    (set, get) => ({
      ...
      settings: resolveSettings(DEFAULT_SETTINGS_MODE, defaultCustomSettings),
      customSettings: defaultCustomSettings,
      setSettingsMode: (settingsMode) => set((state) => ({
        settingsMode,
        settings: resolveSettings(settingsMode, state.customSettings),
      })),
      setSettings: (s) => set((state) => {
        const customSettings = mergeSettings(state.customSettings, s)
        return {
          customSettings,
          settings: resolveSettings(state.settingsMode, customSettings),
        }
      }),
      ...
      moveInputImage: (fromIdx, toIdx) =>
        set((s) => {
          if (fromIdx === toIdx || fromIdx < 0 || fromIdx >= s.inputImages.length) return s

          const images = [...s.inputImages]
          const [moved] = images.splice(fromIdx, 1)
          const insertIdx = Math.max(0, Math.min(images.length, toIdx))
    ...
    }),
    {
      ...
      merge: (persistedState, currentState) => {
        ...
        const customSettings = mergeSettings(
          ...
          typedState?.customSettings ?? typedState?.settings ?? {},
        )
        const settingsMode =
          typedState?.settingsMode === 'public' || typedState?.settingsMode === 'custom'
            ? typedState.settingsMode
            : DEFAULT_SETTINGS_MODE
        ...
        return {
          ...
          settingsMode,
          customSettings,
          settings: resolveSettings(settingsMode, customSettings),
          dismissedCodexCliPrompts: Array.isArray(typedState?.dismissedCodexCliPrompts)
            ? typedState.dismissedCodexCliPrompts
            : [],
        }
      },
    },
...
export async function retryTask(task: TaskRecord) {
  ...
  setTasks([retriedTask, ...tasks])
  await putTask(retriedTask)
  showToast('已重新提交失败任务', 'success')
...
export async function clearAllData() {
  ...
  await clearImages()
  imageCache.clear()
  const { setTasks, clearInputImages, setParams, showToast } = useStore.getState()
  setTasks([])
  clearInputImages()
  useStore.setState({
    settingsMode: DEFAULT_SETTINGS_MODE,
...
export async function exportData() {
  try {
    const tasks = await getAllTasks()
    const images = await getAllImages()
    const { settingsMode, customSettings } = useStore.getState()
    ...
    a.href = url
    a.download = `goodness-images-${Date.now()}.zip`
    a.click()
...
export async function importData(file: File) {
  try {
    ...
    if (data.customSettings || data.settings) {
      useStore.getState().setSettings(data.customSettings ?? data.settings)
    }
    if (data.settingsMode === 'public' || data.settingsMode === 'custom') {
      useStore.getState().setSettingsMode(data.settingsMode)
    }
